...
int Board::eval(bool verbose){
	...
	for (int p = wPa;p <= wPh;p++) {
		...
		if (file == 1) {
			...
			if (blackPawnsOnFile[1] == 0 && blackPawnsOnFile[0] == 0 ) {
				score += PASSEDPAWN;
				if (verbose)
					cout << "Passed a-file ";
			}
			...
			if (board120[pos-9] != none && piece[board120[pos-9]].getValue() == P_VAL) {
				score += PAWNCHAIN;
				if (verbose)
					cout << "Chain a-file ";
			}
			...
			if (blackPawnsOnFile[7] == 0 && blackPawnsOnFile[6] == 0) {
				score += PASSEDPAWN;
				if (verbose)
					cout << "Passed h-file ";
			}
			...
			if (blackPawnsOnFile[file - 2] == 0 && blackPawnsOnFile[file - 1] == 0 && blackPawnsOnFile[file] == 0) {
				score += PASSEDPAWN;
				if (verbose)
					cout << "Passed " << file << "-file ";
			}
			
			if (whitePawnsOnFile[file - 2] == 0 && whitePawnsOnFile[file] == 0) {
				score -= ISOLATEDPAWN;
				if (verbose)
			...
			if (board120[pos - 11] != none && piece[board120[pos - 11]].getValue() == P_VAL) {
				score += PAWNCHAIN;
				if (verbose)
					cout << "Chain " << file << " file ";
			}
    ...
    for (int p = bPa;p <= bPh;p++) {
		...
		if (file == 1) {
			...
			if (whitePawnsOnFile[1] == 0 && whitePawnsOnFile[0] == 0) {
				score -= PASSEDPAWN;
				if (verbose)
					cout << "Passed a-file ";
			}
			...
			if (whitePawnsOnFile[7] == 0 && whitePawnsOnFile[6] == 0) {
				score -= PASSEDPAWN;
				if (verbose)
					cout << "Passed h-file ";
			}
			...
			if (whitePawnsOnFile[file - 2] == 0 && whitePawnsOnFile[file - 1] == 0 && whitePawnsOnFile[file] == 0) {
				score -= PASSEDPAWN;
				if (verbose)
					cout << "Passed " << file << "-file ";
			}
			
			if (blackPawnsOnFile[file - 2] == 0 && blackPawnsOnFile[file] == 0) {
				score += ISOLATEDPAWN;
				if (verbose)
					cout << "Isolated " << file << " file ";
			}
