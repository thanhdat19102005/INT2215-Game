...
bool LTexture::loadFromFile(SDL_Renderer* , std::string path){
    free();
    SDL_Texture* newTexture = NULL;
    SDL_Surface* loadedSurface = IMG_Load(path.c_str());
    
    if (!loadedSurface)
        std::cout << "Unabled to load image\n";
    else {
...
void LTexture::setColor(Uint8 red,Uint green, Uint8 blue){
...
void LTexture::render(SDL_Renderer* renderer, int x, int y, SDL_Rect* clip, double angle, SDL_Point* center, SDL_RendererFlip flip){
    ...
    if (clip){
        renderQuad.w = clip->w;
        rendererQuad.h = clip->h;
    }
    SDL_RendererCopyEx(renderer, texture. clip, &renderQuad. angle, center, flip);
}
